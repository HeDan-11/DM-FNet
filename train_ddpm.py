...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/ddpm_train_MMIF.json',
    ...
    # dataset
    Path_IR = '/media/sata1/hedan/KTZ_DATA/train_13/pair1'
    Path_VIS = '/media/sata1/hedan/KTZ_DATA/train_13/pair2'
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            current_epoch += 1
            for _, (train_data, _, _) in enumerate(train_loader):
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for idx in range(0, opt['datasets']['val']['data_len'], 1):
                        diffusion.test_concat(in_channels=opt['model']['unet']['out_channel'],
                        ...
                        sam_img = Metrics.tensor2img(visuals['SAM'][:, :, :, :])
                        # generation
                        Metrics.save_img(
                            sam_img, '{}/sample_{}_{}_2.png'.format(result_path, current_step, idx),'L')
                        """
                        sam_img = Metrics.tensor2img(visuals['SAM'][:, 0:3, :, :])
                        # generation
                        Metrics.save_img(
                            sam_img, '{}/sample_{}_{}.png'.format(result_path, current_step, idx),'L')
                        """
                        tb_logger.add_image(
